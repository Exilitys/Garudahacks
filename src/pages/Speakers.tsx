...
import { MapPin, Star, DollarSign, Search, Verified, Award } from 'lucide-react';
...
const Speakers = () => {
  ...
  const filteredSpeakers = speakers.filter(speaker => {
    const matchesSearch = speaker.profile?.full_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         speaker.profile?.bio?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         speaker.topics.some(t => t.topic.name.toLowerCase().includes(searchTerm.toLowerCase()));
    const matchesLevel = filterLevel === 'all' || speaker.experience_level === filterLevel;
              ...
              <Card key={speaker.id} className="h-full flex flex-col">
                ...
                <CardContent className="flex-1 flex flex-col justify-between">
                  ...
                  <div className="pt-4 border-t">
                    ...
                    >
                      {speaker.available ? "Contact Speaker" : "Currently Unavailable"}
                    </Button>
                  </div>
                </CardContent>
              </Card>
